package mutator.MutationTestingAppStandalone;
...
public class MutationTestingAppStandalone extends MutatorUtils {
	...
	private int mutation2(List<EPackage> packages, Resource model,
		...
		for (EObject object : objects) {
			String modelsFolder = ModelManager.getModelsFolder(MutationTestingAppStandalone.class);
			...
			if (object != null) {
				...
				if (mutator != null) {
					...
					boolean isRepeated = registryMutantWithBlocksStandalone(ecoreURI, packages, registeredPackages,
							...
							k, mutPaths, hashmapMutVersions, "MutationTestingApp", serialize, test, classes,
							MutationTestingAppStandalone.class, true, false);
					if (isRepeated == false) {
						numMutantsGenerated++;
						monitor.worked(1);
						k[0] = k[0] + 1;
	...
	public int block_m1(int maxAttempts, int numMutants, boolean registry, List<EPackage> packages,
		...
		String modelURI = MutationTestingAppStandalone.class.getProtectionDomain().getCodeSource().getLocation()
				.getPath().replace("/bin/", "/") + "data/model/";
		String modelsURI = MutationTestingAppStandalone.class.getProtectionDomain().getCodeSource().getLocation()
		...
		for (String modelFilename : modelFilenames) {
			...
			numMutants = -1;
			String xmiFilename = MutationTestingAppStandalone.class.getProtectionDomain().getCodeSource().getLocation()
	...
	public MutationResults execute(int maxAttempts, int numMutants, boolean registry, boolean metrics,
		...
		if (metrics == true) {
			...
			metricsGenerator = new NetMutatorMetricsGenerator(
					metricspackages, MutationTestingAppStandalone.class.getProtectionDomain().getCodeSource()
							.getLocation().getPath().replace("/bin/", "/") + "data/out/",
					"/MutationTestingApp/data/model/lib.ecore",
					MutationTestingAppStandalone.class.getProtectionDomain().getCodeSource().getLocation().getPath()
							.replace("/bin/", "/") + "data/model/",
					"MutationTestingApp.mutator", hashmapMutVersions, MutationTestingAppStandalone.class);
