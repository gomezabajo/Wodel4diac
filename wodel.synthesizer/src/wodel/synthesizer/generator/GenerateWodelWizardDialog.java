...
public class GenerateWodelWizardDialog extends TitleAreaDialog implements IWizardContainer2, IPageChangeProvider {
	 ...
	 */
	public GenerateWodelWizardDialog(Shell parentShell, IWizard newWizard) {
		super(parentShell);
	...
	private void savePressed(SelectionEvent e) {
		...
		if (GenerateWodelWizard.configurationFile != null) {
			try {
				File fileAccess = new File(GenerateWodelWizard.configurationFile);
				FileWriter fileWriter = new FileWriter(fileAccess);
				fileWriter.append(String.format("%d", GenerateWodelWizard.numSeeds) + "\n");
				fileWriter.append(GenerateWodelWizard.forceRoot + "\n");
				String selectedBlockNamesLine = "";
				if (GenerateWodelWizard.blockNames != null) {
					for (String selectedBlockName : GenerateWodelWizard.blockNames) {
						selectedBlockNamesLine += selectedBlockName + ",";
					}
					if (selectedBlockNamesLine.length() > 0) {
						selectedBlockNamesLine = selectedBlockNamesLine.substring(0, selectedBlockNamesLine.lastIndexOf(","));
					}
					fileWriter.append(selectedBlockNamesLine + "\n");
				}
				if (GenerateWodelWizard.classNames != null) {
					for (String className : GenerateWodelWizard.classNames) {
						if (GenerateWodelWizard.numObjects.containsKey(className)) {
							fileWriter.append(className + " # " + GenerateWodelWizard.numObjects.get(className) + "\n");
						}
					}
					for (String className : GenerateWodelWizard.classNames) {
						if (GenerateWodelWizard.tagsByClass.containsKey(className)) {
							fileWriter.append(className + "(" + GenerateWodelWizard.tagsByClass.get(className).getKey() + "): " + GenerateWodelWizard.tagsByClass.get(className).getValue() + "\n");
						}
					}
				}
				if (GenerateWodelWizard.initialPath != null) {
					fileWriter.append("initial: " + GenerateWodelWizard.initialPath + "\n");
				}
				if (GenerateWodelWizard.customOCL != null) {
					fileWriter.append("ocl: " + GenerateWodelWizard.customOCL + "\n");
				}
				fileWriter.flush();
				fileWriter.close();
	...
	protected void createButtonsForButtonBar(Composite parent) {
		...
		createLoadAndSaveButtons(parent);

		createFinishAndCancelButton(parent);
	}
	 ...
	 */
	private Composite createFinishAndCancelButton(Composite parent) {
		...
		finishButton = createButton(composite, IDialogConstants.FINISH_ID, IDialogConstants.FINISH_LABEL, true);
		finishButton.setEnabled(true);
		cancelButton = createButton(composite, IDialogConstants.CANCEL_ID, IDialogConstants.CANCEL_LABEL, false);
	 ...
	 * @return a composite containing the new buttons
	 */
	private Composite createLoadAndSaveButtons(Composite parent) {
		// increment the number of columns in the button bar
		((GridLayout) parent.getLayout()).numColumns++;
		Composite composite = new Composite(parent, SWT.NONE);
		// create a layout with spacing and margins appropriate for the font
		...
		composite.setLayoutData(data);
		composite.setFont(parent.getFont());
		loadButton = createButton(composite, 22, "Load", false);
		SelectionListener loadListener = widgetSelectedAdapter(e -> loadPressed(e));
		loadButton.addSelectionListener(loadListener);

		loadButton.setEnabled(true);
		saveButton = createButton(composite, 23, "Save", false);
		SelectionListener saveListener = widgetSelectedAdapter(e -> savePressed(e));
