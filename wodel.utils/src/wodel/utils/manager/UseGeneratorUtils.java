...
public class UseGeneratorUtils {
	...
	private static final String USE_KEYWORD_PREFIX = "xxxx";
	private static final Set<String> USE_KEYWORDS = java.util.Collections.unmodifiableSet(new java.util.HashSet<>(Arrays.asList(useWords)));
	...
	public static String getTarUseReference(EReference ref, Map<URI, Map<URI, Entry<String, String>>> uriUseReferences) {
		...
		final EClass container = (EClass) ref.eContainer();
		final URI containerUri = EcoreUtil.getURI(container);
		final URI refUri = EcoreUtil.getURI(ref);
		final Map<URI, Entry<String, String>> byRef = uriUseReferences.get(containerUri);
		final String name;
		if (byRef != null) {
			final Entry<String, String> entry = byRef.get(refUri);
			name = (entry != null) ? entry.getValue() : ref.getName();
		} else {
			name = ref.getName();
		}
	...
	public static String getSrcUseReference(EReference ref, Map<URI, Map<URI, Entry<String, String>>> uriUseReferences) {
		...
		if (byRef != null) {
			final Entry<String, String> entry = byRef.get(refUri);
			name = (entry != null) ? entry.getKey() : ref.getName();
		} else {
			name = ref.getName();
		}
		return encodeWord(name);
	...
	public static String encodeWord(String word) {
		...
		return USE_KEYWORDS.contains(word) ? (USE_KEYWORD_PREFIX + word) : word;
	}
	...
	public static String decodeWord(String word) {
		...
		if (word.startsWith(USE_KEYWORD_PREFIX)) {
			final String candidate = word.substring(USE_KEYWORD_PREFIX.length());
			return USE_KEYWORDS.contains(candidate) ? candidate : word;
		}
		return word;
	}
	 ...
	 * @return
	 */
	public static String ensureContainer(List<EPackage> packages, List<EClass> classes,
		...
		for (EClass eClass : classes) {
			...
			final URI eClassUri = EcoreUtil.getURI(eClass);
			final String className = classNames.get(eClassUri);
			if (className == null) {
				continue;
			}
			final List<EReference> references = new ArrayList<EReference>();
			final List<EClassifier> classifiers = ModelManager.getContainerTypes(packages, eClassUri);
			if (classifiers.size() > 1) {
				for (EClassifier classifier : classifiers) {
					if (!(classifier instanceof EClass)) {
						continue;
					}
					final EClass container = (EClass) classifier;
			...
			if (references.size() > 1) {
				constraints.append("\n\ncontext ")
				.append(encodeWord(className))
				.append(" inv ensure_container:\n\t(");
				...
				for (EReference ref : references) {
					...
					for (EReference reff : references) {
						final String role = getSrcUseReference(reff, useReferences);
						...
						if (EcoreUtil.equals(reff, ref)) {
							constraints.append("not ").append(role).append(".oclIsUndefined()");
						} else {
							constraints.append(role).append(".oclIsUndefined()");
						}
					}
				}
				constraints.append(")\n");
	...
	public static String compositionConstraint (List<EPackage> packages, List<EReference> references,
			Map<URI, Map<URI, Entry<String, String>>> useReferences, Map<URI, String> classNames) {
		final StringBuilder constraints = new StringBuilder();

		// obtain the containment references that can contain each class
		final Map<EClass, List<EReference>> containers = new HashMap<EClass, List<EReference>>();
		for (EReference ref : references) {
			if (ref == null) {
				continue;
			}
			if (ref.isContainment()) {
				...
				for (EClass type : types) {
					if (type != null && !type.isAbstract()) {
						List<EReference> list = containers.get(type);
						if (list == null) {
							list = new ArrayList<EReference>();
							containers.put(type, list);
						}
						list.add(ref);
					}
				}
			...
			if (ref.getEOpposite() != null && ref.getEOpposite().isContainment()) {
				...
				for (EClass type : types) {
					if (type != null && !type.isAbstract()) {
						...
						list.add(ref.getEOpposite());
					}
				}
			}
		}

		// if a class can potentially be in more than one container, add a constraint
		for (Entry<EClass, List<EReference>> entry : containers.entrySet()) {
			final List<EReference> refs = entry.getValue();
			if (refs == null || refs.size() <= 1) {
				continue;
			}
			final String classname = classNames.get(EcoreUtil.getURI(entry.getKey()));
			if (classname == null) {
				continue;
			}
			constraints.append("\n\ncontext ").append(encodeWord(classname)).append(" inv single_container:\n");
			boolean first = true;
			for (EReference ref : refs) {
				final String containingClassName = classNames.get(EcoreUtil.getURI(ref.getEContainingClass()));
				...
				first = false;
				constraints.append("\t")
				.append(encodeWord(containingClassName))
				.append(".allInstances()->collect(o | o.")
				.append(getTarUseReference(ref, useReferences))
				.append(")->count(self)");
			}
	...
	public static String compositionConstraint (EReference ref, Map<URI, Map<URI, Entry<String, String>>> useReferences, Map<URI, String> classNames) {
		...
		if (!(ref.getEContainingClass() == ref.getEReferenceType() || ref.getEContainingClass().getESuperTypes().contains(ref.getEReferenceType()))) {
			return "";
		}
		if (classNames.get(EcoreUtil.getURI(ref.getEContainingClass())) == null) {
			return "";
		...
		final List<String> terms = new ArrayList<String>(numTerms);
		for (int term = 1; term <= numTerms; term++) {
			String expression = "";
			for (int index = 1; index <= term; index++) {
				if (index == 1) {
					expression = setOpen + (index < term ? (role + index + ".oclAsType(" + type + ").") : "self.")
							+ role + setClose + "->includes(self)";
				} else {
					String select1 = "";
					String select2 = "";
					select1 = setOpen + (index < term ? (role + index + ".oclAsType(" + type + ").") : "self.")
							+ role + setClose + "->exists(" + role + (index - 1) + " |\n";
					select2 = ")";
					expression = select1
							+ "if " + role + (index - 1) + ".oclIsKindOf(" + type + ") then\n"
							+ "\t" + expression + "\n"
							+ "\t else false endif"
							+ select2;
				}
		...
		constraint.append("\n\ncontext ")
		.append(encodeWord(className))
		.append(" inv non_contains_itself_")
		.append(role)
		.append(":\n");
		for (int i = 0; i < terms.size(); i++) {
			constraint.append(terms.get(i)).append("\n");
			if (i < terms.size() - 1) {
	...
	private static Constraint unite(List<Constraint> constraints, Constraint constraint) {
		if (constraint == null || constraint.text == null || constraint.text.isEmpty()) {
			return constraint;
		}
		for (Constraint c : constraints) {
			if (!"exists".equals(c.type) || !c.list || c.clauses.size() != 1) {
				continue;
			}
			if (!"exists".equals(constraint.type) || constraint.list) {
				continue;
			}
			if (c.className == null || !c.className.equals(constraint.className)) {
				continue;
			}
			if (c.text == null || c.text.equals(constraint.text)) {
				continue;
			}
			// Avoid duplicates
			for (String clause : c.clauses) {
				if (constraint.text.equals(clause)) {
					return c;
				}
			}

			final int totalVars = constraint.variables.size() + c.variables.size();
			final String[] variables = new String[totalVars];
			final Map<String, String> constraintVars = new HashMap<String, String>();
			int idx = 0;
			for (String v : constraint.variables) {
				variables[idx] = v.substring(0, 1) + idx;
				constraintVars.put(v, variables[idx]);
				idx++;
			}
			final Map<String, String> cVars = new HashMap<String, String>();
			for (String v : c.variables) {
				variables[idx] = v.substring(0, 1) + idx;
				cVars.put(v, variables[idx]);
				idx++;
			}

			final StringBuilder clause = new StringBuilder();
			clause.append(c.className).append(".allInstances()->exists(");
			for (String v : variables) {
				clause.append(v).append(", ");
			}
			clause.setLength(clause.length() - 2); // drop trailing ", "
			clause.append(" | ");

			final String v1 = variables[0];
			for (int i = 1; i < variables.length; i++) {
				clause.append(v1).append(" <> ").append(variables[i]).append(" and ");
			}

			String clauseConstraint = constraint.text.substring(constraint.text.indexOf("|") + 2, constraint.text.lastIndexOf(")"));
			for (Entry<String, String> e : constraintVars.entrySet()) {
				clauseConstraint = clauseConstraint.replace(e.getKey() + ".", e.getValue() + ".");
			}

			String clauseC = c.text.substring(c.text.indexOf("|") + 2, c.text.lastIndexOf(")"));
			for (Entry<String, String> e : cVars.entrySet()) {
				clauseC = clauseC.replace(e.getKey() + ".", e.getValue() + ".");
			}

			clause.append("(").append(clauseConstraint).append(") and (").append(clauseC).append("))");
			...
			newConstraint.text = clause.toString();
	...
	public static String generateUSE(Resource model, MutatorEnvironment e, String modelName, Map<URI, Map<URI, Entry<String, String>>> useReferences) {
		...
		try {
					...
					for (EReference ref : refs) {
						if (classNames.get(EcoreUtil.getURI(ref.getEType())) != null) {
							...
							if (ref.isContainment() == false) {
								useText.append("association " + encodeWord(associationName) + " between\n");
								useText.append("\t" + encodeWord(clName) + "[*] role " + encodeWord(srcRoleName) + "\n");
							}
							if (ref.isContainment() == true) {
								useText.append("composition " + encodeWord(associationName) + " between\n");
								useText.append("\t" + encodeWord(clName) + "[0..1] role " + encodeWord(srcRoleName) + "\n");
							}
							if (ref.getLowerBound() == 0 && ref.getUpperBound() == -1) {
								useText.append("\t" + encodeWord(refETypeName) + "[*] role " + encodeWord(tarRoleName) + "\n");
							}
							else if (ref.getUpperBound() == -1) {
								useText.append("\t" + encodeWord(refETypeName) + "[" + ref.getLowerBound() + "..*] role " + encodeWord(tarRoleName) + "\n");
							}
							else if (ref.getLowerBound() != ref.getUpperBound()) {
								useText.append("\t" + encodeWord(refETypeName) + "[" + ref.getLowerBound() + ".." + ref.getUpperBound() + "] role " + encodeWord(tarRoleName) + "\n");
							}
							else if (ref.getLowerBound() == ref.getUpperBound()) {
								useText.append("\t" + encodeWord(refETypeName) + "[" + ref.getLowerBound() + "] role " + encodeWord(tarRoleName) + "\n");
							}
			...
			for (Constraint constraint : constraints) {
				i++;
				if (constraint.type.equals("size")) {
					useText.append("inv mut" + i + " : " + constraint.text + " > " + maxSize(constraint) + "\n");
					maxSize.put(constraint.text.substring(0, constraint.text.indexOf(".")), maxSize(constraint) + 1);
				}
				else {
					useText.append("inv mut" + i + " : " + constraint.text + "\n");
				}
			}
	...
	//				}
	//				
	//				//String complexityLevelOCLFile = ModelManager.Platform.getLocation().getPath().toString().replace("\\", "/") + "/" + WodelContext.getProject() + "/data/ocl/" + packages.get(0).getName() + "." + complexityLevel.toLowerCase() + ".txt";
	//
	...
	//				String xmiFileName = "file:/" + ModelManager.Platform.getLocation().getPath().toString().replace("\\", "/") + "/" + WodelContext.getProject() +
