...
class EduTestMoodleGeneratorAlternativeText {
	var AlternativeText exercise
	...
	def String setUpAndCompile(EduTestSuperGenerator generator, AlternativeText exercise, Program program, Resource resource, List<Integer> li, List<EClass> roots) {
...
<!--«text = generator.getText((exercise as AlternativeText).config.identifier, ModelManager.getMetaModelPath() + "/" + test.source, resource)»-->
«/*ELSE*/»
<!--«/*text = generator.getText((exercise as AlternativeText).config.identifier, ModelManager.getOutputPath() + "/" + test.source.replace('.model', '') + "/" + diagram.replace(".png", ".model"), resource)*/»-->
«/*ENDIF*/»
<text><![CDATA[<p>«test.question.replace("\"", "'")»<br></p><p><img width="20%" height="20%" src="data:image/png;base64,«generator.getStringBase64("diagrams/" + test.source.replace('.model', '') + "/" + diagram)»"><br></p><!--<p><img src="@@PLUGINFILE@@/exercise_«uuid».png" alt="«test.question.replace("\"", "'")»" width="20%" height="20%" class="img-responsive atto_image_button_text-bottom"><br></p><p>«text»<br></p>-->]]></text>
<!--<file name="exercise_«uuid».png" path="/" encoding="base64">«generator.getStringBase64("diagrams/" + test.source.replace('.model', '') + "/" + diagram)»</file>-->
</questiontext>
