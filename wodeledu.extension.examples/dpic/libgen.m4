...
`#' `$1' Version 11.0.4: ifelse(m4picprocessor,gpic,`Gpic',
...
define(`Point_',`ifinstr(`$2',D,`ifelse(dnl set rp_ang and m4_dir_ (manhattan)
`$1',0,`rp_ang = 0; right_',
...
define(`setdir_',
 `pushdef(`m4sdir',`ifelse(`$1',,
  ...
  m4sd_ang = rp_ang; pushdef(`m4tdir',m4_dir_)dnl save current dir
ifinstr(
...
pushdef(`dashline',`line invis `$1'; M4S:last line.start; M4E:last line.end
 ...
 {if dashline_n==1 then { line `$2' to M4S+M4dsh } \
  ...
  line invis to M4E popdef(`m4gap',`m4dsh',`m4opts') ')
                               ...
                               `round(at position,line thickness,colorspec)
                                filled circle for rounded corners'
define(`round',`{ circle thick ifelse(`$2',,linethick,(`$2'))/2 \
...
`ifelse(substr(`$1',0,1),ifelse(`$2',,_,`$2'),`substr(`$1',1)',`$1')')
                                ...
                                `pushkey_(string,key,default,[N])
                                 ...
                                 expr and default val may not contain ;'
define(`pushkey_',
...
define(`dot',`[ pushdef(`m4ft',`ifelse(`$3',,0,(`$3'))')dnl
  ...
  `$4' popdef(`m4rad',`m4circle',`m4ft')] ifelse(`$1',,`at Here',`$1')
  move to last []')
  ...
  popdef(`m4attrib',`m4rad',`m4shadowthick',`m4shadowcolor',`m4shadowangle') ')
                               ...
                               `hatchbox(boxspec,hatchsep,hatchspec,angle)
                                ...
                                keys:
                                  ...
                                  angle=degrees;
                                  boxdir=degrees; (must be manhattan)'
...
B: box ifinstr(m4_dir_,`t',`ht m4wid wid m4ht',`wid m4wid ht m4ht') \
    ifinstr(`$1',=,,`$1') invis with .c at (0,0)
  ifinstr(`$1',shade,
  ...
  if abs(m4_a) < 0.1 then { for y = m4_hs to B.ht by m4_hs do { # hatchangle=0
    ...
    line from vec_(U.x,U.y) to vec_(V.x,V.y) m4hatchspec } } \
  else { m4_d = max(B.wid,B.ht)/5  # general hatch ang
    dnl                     DT is hatchline rotated -90 deg
    T: B.nw; DT: (m4_sa,-m4_ca)*m4_hs; DQ: (m4_ca,m4_sa)*m4_d
    for m4_lp=1 to 0 by -1 do { T: T+DT; Q: T+DQ
      S: (B.w.x, intersect_(T,Q,B.nw,B.sw).y) # left-side (or bottom) intersect
      if S.y < B.s.y then { S: (intersect_(T,Q,B.sw,B.se).x, B.s.y) }
      F: (intersect_(T,S,B.nw,B.ne).x, B.n.y) # top (or right-side) intersect
      if F.x > B.e.x then { F: (B.e.x, intersect_(T,S,B.ne,B.se).y) }
      m4_lp = (F.y >= B.s.y && F.x <= B.e.x) # isinbox, draw then loop
  ...
  resetdir_; popdef(`m4wid',`m4ht',`m4box',`m4hatchsep',`m4hatchspec',
   `m4angle',`m4boxdir') ]' )
                                ...
                                `lbox(wid,ht,attributes,[r=expr])
                                 box oriented in current direction;
                                 Arg3= eg dashed shaded "red"
                                 Arg4 specifies corner radius'
define(`lbox',`pushdef(`m4bw2',ifelse(`$1',,boxwid/2,(`$1')/2))dnl
  pushdef(`m4bh2',ifelse(`$2',,boxht/2,(`$2')/2))dnl
  ifinstr(`$4',r=,`pushkey_(`$4',r,0)dnl
    br_lbx = Min(m4r,m4bh2,m4bw2) popdef(`m4r')
    C_lbx[0]: (m4bw2-br_lbx,m4bh2-br_lbx)
    C_lbx[1]: (-m4bw2+br_lbx,m4bh2-br_lbx)
    C_lbx[2]: (-m4bw2+br_lbx,-m4bh2+br_lbx)
    C_lbx[3]: (m4bw2-br_lbx,-m4bh2+br_lbx)
    pts_lbx = 7
    for j=0 to 3 do {
      for i=0 to pts_lbx do { T_lbx: C_lbx[j]+(Rect_(br_lbx,j*90+i*90/pts_lbx))
        P_lbx[i]: rvec_(m4bw2,0)+vec_(T_lbx.x,T_lbx.y) }
        fitpoints(P_lbx,pts_lbx,0,P_lbxf,j*(pts_lbx+1)) }
    P_lbxf[-1]: rvec_(m4bw2*2,0)
    S_lbx: spline ctension_ `$3' from P_lbxf[-1] to P_lbxf[0]
    for i=1 to 4*(pts_lbx+1)-1 do { continue to P_lbxf[i] }
    continue to P_lbxf[-1] ',
 `line from rvec_(m4bw2*2,0) \
    to rvec_(m4bw2*2,m4bh2) \
    then to rvec_(0,m4bh2) \
    then to rvec_(0,neg_(m4bh2)) \
    then to rvec_(m4bw2*2,neg_(m4bh2)) \
    then to rvec_(m4bw2*2,0) `$3' dnl
    popdef(`m4bw2',`m4bh2')') ')

                                `rotbox(wid,ht,attribs,[r=expr|t=expr])
...
define(`m4lstring',`ifelse(
 index(`$1',"),0,`$1',index(`$1',sprintf),0,`$1',`$2')')
                       ...
                       `Dimensioning for diagrams:
                        dimension_(linespec, sideways offset, label,
...
define(`Equidist3',`
  ...
  ifelse(`$5',,,`$5 = distance(`$4',`$1');') ')
                           ...
                           `incircle(A,B,C,Ctr,rc)
                            Centre Ctr and radius rc of circle inscribed in
                            triangle A,B,C'
                           `e.g. incircle(A,B,C,Ctr,rc); circle rad rc at Ctr'
define(`incircle',
 `a_incirc = (langle(`$1',`$2')+langle(`$1',`$3'))/2
  b_incirc = (langle(`$2',`$3')+langle(`$2',`$1'))/2
  `$4': intersect_(`$1',(`$1'+(rect_(1,a_incirc))),
                   `$2',(`$2'+(rect_(1,b_incirc))))
  P_incirc: PerpTo(`$4',`$1',`$2')
  `$5' = distance(`$4',P_incirc);')
...
ifdpic(`
define(`sinusoid',
 `pushdef(`m4s_a',`ifelse(`$1',,linewid,(`$1'))')dnl
  pushdef(`m4s_ph',`ifelse(`$3',,0,(`$3'))')dnl
  pushdef(`m4s_m',`ifelse(`$4',,0,(`$4'))')dnl
  pushdef(`m4s_M',`ifelse(`$5',,linewid,(`$5'))')dnl
  [ Origin: (0,0)
    Tmin: vec_(m4s_m,0) ; Tmax: vec_(m4s_M,0)
    h = m4s_M-m4s_m
    ifelse(`$2',,`if h!=0 then {f=twopi_/h} else {f=twopi_}',`f = `$2'')
    tm = 2*f
    ...
    End: M4S_[n]
    fitcurve(M4S_,n,`$6')
  `$7' popdef(`m4s_a',`m4s_ph',`m4s_m',`m4s_M') ]')
  define(`m4_t_fun',
  ...
  `$4' popdef(`m4obj') ]')
                                ...
                                `rgbstring(color triple with values in [0,1])
                                 ...
                                 `This macro can be fragile when used as
                                  an m4 macro argument. A pic macro will
                                  delay expansion:
