...
public class MyModelUtils {
	 ...
	 * 
	 * @param resource
	 *            {@link org.eclipse.emf.ecore.resource.Resource resource} loaded model.
	 * @param resourceSet
	 *            The {@link ResourceSet} to load the model in.
	 * @param resolve
	 *            If true the method will resolve the proxies in the loaded resource.
	 ...
	 * @since 3.5
	 */
	public static EObject load(Resource resource, boolean serialized) throws IOException {
		EObject result = null;
		try {
			...
			if (serialized == true) {
				...
				String resourceString = outputStream.toString();
				modelResource.load(new URIConverter.ReadableInputStream(resourceString), null);
			}
			...
			for (Diagnostic diagnostic : errors) {
				if (diagnostic != null) {
					String errorMessage = AcceleoCommonMessages.getString(LOADING_ERROR_KEY, resource.getURI().toString());
					errorMessage += '\n' + diagnosticString(diagnostic);
					AcceleoLogger.log(errorMessage, false);
				}
			}
